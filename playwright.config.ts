...
export default defineConfig({
	...
	/*
	 * Три раскладки — это про КАРТИНКУ, а не про поведение.
	 *
	 * Раньше в трёх окнах гонялось всё подряд, и три четверти прогона уходило
	 * на повторение того, что от ширины окна не зависит вовсе: рейкаст по
	 * вилке, цвета в readPixels, ползунки стендов, набор ссылок в шапке.
	 * Самый долгий тест во всём наборе — рывок за шнур, двадцать секунд, — так
	 * жил в трёх экземплярах, отличавшихся только тем, у какой стенки стоит
	 * телевизор.
	 *
	 * Поэтому снимки помечены тегом @shot и снимаются во всех трёх окнах, а
	 * всё остальное идёт один раз в самом широком: там телевизор ближе всего
	 * к стенке сцены, то есть физике теснее, чем в двух других.
	 */
	projects: [
		{
			name: 'поведение',
