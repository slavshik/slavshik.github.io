...
/*
 * Здесь когда-то мерили ореол: что размытие не съедает полутона внутри
 * картинки, не светит из-за краёв экрана и не рисуется, когда трубка
 * отвёрнута от камеры. Ореола больше нет — ни в спеке, ни в коде, — и от той
 * проверки осталось то, что от него не зависело и больше нигде не
 * проверяется: шкала яркости через шейдер экрана (эталоны ловят на ней одну
 * точку, а не монотонность) и отражение среды в стекле — единственное, что
 * отличает стекло от куска пластика, и единственный потребитель карты среды.
 */
