...
- `src/tv/` — the CRT television, fourteen modules. `index.ts` exports `mount()`; `physics.ts`, `constants.ts`, `look.ts` and `shaders.ts` contain no three and no WebGL, which is what makes the physics unit-testable. `lab.ts` is the workbench control surface and must stay out of the production chunk.
...
- **`make stats` is where the visits are read.** `scripts/stats.mjs` sends eight SELECTs to D1 in one `wrangler d1 execute` — a call costs seconds, so eight of them would be a wait rather than a summary — and prints days, pages, countries, referrers, browsers, the reasons the television stayed away, and today line by line. `make stats DAYS=30` widens the window. Two counting rules live in that file and nowhere else: a row with `tex=1` is a request for a clip, not a visit, and a row with `x=err` is a report, not a visit — count either and the traffic multiplies by how long somebody watched. It also names the robots: a visitor whose browser calls itself a recent Chrome or Firefox and then reports no WebGL2 is lying about one of the two, and that line is what explains a day where «people» suddenly trebles. Times are local: `at` is UTC and the shift comes from this machine. The CV counts itself into the same table from the neighbouring `cv` repository (`src/hit.ts` there), so `/cv/` shows up under «страницы» beside `/`.
- **The Television is draggable by both ends.** The body is grabbed through a box proxy, as it always was; the Plug is grabbed through a sphere proxy of its own, checked first because it is the smaller target. The Rope does not stretch: the Plug stops at the length of the flex and everything past that becomes Tension on the Anchor, which is at the bottom of the back wall and therefore tips the set as well as sliding it. Pull hard enough and the Television leaves the floor and hangs from its own cord — which puts it in the air, which is what makes the screen fall back to Snow. A sharp yank flashes the screen and changes the channel; a smooth pull does neither, and that distinction is a threshold on the _rise_ of the tension, not on the tension itself. Test it from the Lab with «Дёрнуть за вилку» — mouse gestures cannot be aimed at the Plug without knowing where the physics parked it.
