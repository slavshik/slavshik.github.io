 ...
 *
 * Считается так: поле высот из двух октав значения-шума, затем нормали
 * конечными разностями по нему. Шум берётся с решётки, замкнутой по модулю,
 * поэтому плитка сходится сама с собой и повторяется по корпусу без шва.
 */
export interface GrainSpec {
	/** Сторона плитки в текселях. */
	size: number;
	/** Ячеек шума на сторону: чем больше, тем мельче зерно. */
	cells: number;
