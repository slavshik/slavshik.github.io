...
export interface GrainSpec {
	...
	size: number;
	/**
	 * Ячеек на сторону плитки: чем больше, тем мельче зерно.
	 *
	 * Считать надо вместе с repeat и с размером грани на экране. Грань корпуса
	 * там около сотни пикселей, и когда cells·repeat переваливает за сотню,
	 * на ячейку приходится меньше пикселя — мипмап усредняет рельеф в гладь, и
	 * зерна не видно вовсе. Проверено дважды: 26×3 и 34×3 не дали ничего.
	 */
	cells: number;
	/** Крутизна склонов в поле высот — «глубина» шероховатости. */
	relief: number;
	/** Во сколько раз плитка повторяется по грани корпуса. */
	repeat: number;
