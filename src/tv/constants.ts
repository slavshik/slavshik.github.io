...
export interface TvParams {
	...
	ropeG: number;
	ropeDamp: number;
	/* Кручение провода: своя свитость шнура, жёсткость и вязкость
	   крутильной цепочки, момент инерции вилки на её конце. */
	ropeCurl: number;
	twistK: number;
	twistC: number;
	plugInertia: number;
	/* Во сколько раз преувеличить кручение на экране. Не физика — оптика. */
	twistGain: number;
	/** Полуразмер вилки для столкновения с корпусом. */
	plugPad: number;
	dropY: number;
	homeGap: number;
	floorGap: number;
...
export const DEFAULTS: TvParams = {
	...
	ropeCurl: 0.012, // насколько шнур свит: кривизна покоя, уводящая его из плоскости
	twistK: 90.0, // жёсткость провода на кручение
	twistC: 1.6, // вязкость кручения
	plugInertia: 3.2, // момент инерции вилки против звена провода
	twistGain: 12.0, // преувеличение кручения на экране; см. комментарий в index.ts
	plugPad: 0.11, // полуразмер вилки: она толще звена и цепляет корпус раньше
