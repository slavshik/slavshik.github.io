...
export interface TvParams {
	...
	plugPad: number;
	/* Шнур, когда за вилку тянут: жёсткость натянутого шнура, вязкость,
	   потолок силы и доля, уходящая в разворот корпуса. */
	cordK: number;
	cordC: number;
	cordMax: number;
	cordLever: number;
...
export const DEFAULTS: TvParams = {
	...
	antK: 30.0, // жёсткость антенн — мягче, чтобы болтались смешнее
	antC: 2.2, // ζ≈0.20: рожок успевает качнуться раз пять, прежде чем встать
	antLever: 0.8, // отклик на разгон вращения корпуса
	antLift: 0.8, // отклик на разгон корпуса по прямой — им и живёт подскок
	// Сухое трение в шарнире. Держать его ниже antK * 0.01: рожок замирает
	// там, где пружина уже не может его сдвинуть, и эта полоса обязана быть
	// уже порога тишины в условии сна — иначе телевизор не заснёт никогда.
	antDry: 0.18,
	ropeG: 18.0, // гравитация проводка
	ropeDamp: 0.988, // сохранение скорости в верле
	ropeCurl: 0.006, // насколько шнур свит: кривизна покоя, уводящая его из плоскости.
	// Она же кормит привод кручения, поэтому распрямить шнур — значит заодно
	// ослабить закрутку вилки: два числа связаны, крутить их врозь не выйдет.
	...
	twistGain: 9.0, // преувеличение кручения на экране; см. комментарий в index.ts
	plugPad: 0.11, // полуразмер вилки: она толще звена и цепляет корпус раньше
	// Шнур нерастяжим, поэтому корпус он начинает тянуть ровно тогда, когда
	// распрямился, — до этого пружина молчит. Жёсткости хватает, чтобы
	// перебороть тяжесть (gravity 22) при перетяге примерно в четверть
	// единицы: телевизор можно поднять за провод, но не одним касанием.
	cordK: 120.0,
	cordC: 4.0, // вязкость: без неё натянутый шнур звенит, как струна
	cordMax: 90.0, // потолок силы — около четырёх g, дальше это уже не шнур
	cordLever: 1.0, // якорь сидит снизу-сзади, поэтому натяжение ещё и кренит
